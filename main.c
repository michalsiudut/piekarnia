...
int main()
{
    ...
    int msgid_kasjer = msgget(KEY_MSG_KLIENT_KASJER, IPC_CREAT | IPC_EXCL | 0600); // twprzenie kolejki kom z klienta do kasjera
    if (msgid_kasjer < 0)
    {
    ...
    int sem_id = semget(SEM_KEY_DO_SKLEPU, 1, IPC_CREAT | 0600);
    if (sem_id == -1)
    {
